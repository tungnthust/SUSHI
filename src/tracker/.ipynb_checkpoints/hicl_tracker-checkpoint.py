...
class HICLTracker:
    ...
    def _get_model(self):
        ...
        # ReID Node Dimension, default 2048
        ...
        return HICLNet(submodel_type=MOTMPNet, 
                       submodel_params=mpntrack_params['graph_model_params'],
                       hicl_depth=self.config.hicl_depth, #9
                       use_motion=self.config.mpn_use_motion, # Use motion predictions GIoU as edge feature at each layer, default=[False, True, True, True, True, True, True, True, True]
                       use_reid_edge=self.config.mpn_use_reid_edge, # Use ReID of node as edge feature, default=[True]*9
                       use_pos_edge=self.config.mpn_use_pos_edge, # Use position of object as edge feature, default=[True]*9
                       share_weights=self.config.share_weights, # share weight between graph level, default = 'all_but_first's
                       edge_level_embed=self.config.edge_level_embed, # True
    ...
    def train(self, train_dataset, validation_dataset=None):
        ...
        for epoch in range(1, self.config.num_epoch+1):
            ...
            if validation_dataset and epoch >= self.config.start_eval:
                epoch_val_logs, epoc_val_logs_per_depth, seq_output_df = self.track(dataset=validation_dataset, output_path=epoch_path, mode='val', oracle=False)
    ...
    def track(self, dataset, output_path, mode='val', oracle=False):
        ...
        return logs_total, logs_all, seq_output_df
